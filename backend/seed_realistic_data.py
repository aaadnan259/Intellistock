...
def seed_realistic():
    ...
    for cat, traits in categories.items():
        for i in range(10):
            ...
            if cat == "Food":
                base_cost = random.randint(2, 20)
            if cat == "Furniture":
                base_cost = random.randint(100, 2000)

            price = round(base_cost * traits["margin"], 2)
            name = (
                f"{cat} Item {i+1} - "
                f"{random.choice(['Pro', 'Max', 'Lite', 'Standard', 'Premium'])}"
            )
    ...
    for item in products_db:
        ...
        if cat == "Food":
            avg_sales = random.randint(20, 100)
        if cat == "Furniture":
            avg_sales = random.randint(0, 3)  # Slow mover
