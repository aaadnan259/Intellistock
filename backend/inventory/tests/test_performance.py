...
class TestPerformance:
    ...
    def test_product_list_performance_1k(self):
        # Seed 1000 products
        products = [
            Product(name=f"Product {i}", sku=f"SKU-{i}", price=10.0, current_stock=100)
            for i in range(1000)
        ]
    ...
    def test_forecast_performance(self):
        ...
        p = Product.objects.create(
            name="Forecast Test", sku="F-TEST", price=100, current_stock=50
        )
        ...
        response = self.client.post(
            "/api/forecasting/advanced-predict/",
            {"product_id": p.id, "days": 30, "model": "auto"},
        )
        end_time = time.time()
